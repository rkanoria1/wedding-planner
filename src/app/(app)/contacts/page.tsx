    ...
    <div className="mx-auto max-w-4xl space-y-5">
      <div className="card-lux relative overflow-hidden p-6">
        ...
        <div className="relative flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-muted-foreground">
          <PhoneCall className="size-4 text-gold" /> {tr("contacts.eyebrow", "Day-of Contacts")}
        </div>
        <h1 className="relative mt-1 font-display text-3xl">
          {tr("page.contacts.title", "Who to call, in one tap")}
        </h1>
        <p className="relative mt-1 max-w-lg text-sm text-muted-foreground">
          {tr(
            "contacts.sub",
            "Every vendor and point-of-contact you've saved, grouped by function — so on the day, anyone can call or WhatsApp without hunting for numbers."
          )}
        </p>
      </div>
      ...
      {total === 0 ? (
        <EmptyState
          icon={PhoneCall}
          title={tr("contacts.empty.title", "No contacts yet")}
          hint={tr(
            "contacts.empty.hint",
            "Add a phone number to your bookings or vendors and they'll appear here automatically."
          )}
        />
      ) : (
        groups.map((g) => (
          <section key={g.id} className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-xl">
                {g.id === "general" ? tr("misc.general", "General") : g.name}
              </h2>
                ...
                >
                  ...
                  <div className="flex shrink-0 items-center gap-1.5">
                    <a
                      ...
                      aria-label={`${tr("action.call", "Call")} ${c.name}`}
                    >
                    ...
                    <a
                      href={whatsappLink(
                        c.phone,
                        tr("wa.contact", "Hi{who}! Regarding {role} for Rahul & Somya's wedding —", {
                          who: c.person ? ` ${c.person}` : "",
                          role: c.role,
                        })
                      )}
                      target="_blank"
                      ...
                      aria-label={`${tr("action.whatsapp", "WhatsApp")} ${c.name}`}
                    >
